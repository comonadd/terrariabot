...
class Client:
    ...
    def stop(self):
        # TODO: Fix this being called when not supposed to stop
        print('stopping')
        self.running = False
    ...
    def got_world_info(self, msg):
        ...
        if self.player.initialized and not self.player.spawned:
            ...
            self.add_message(SpawnPlayer(self.player,
    ...
    def did_spawn(self):
        return self.player and self.player.initialized and self.player.spawned
    ...
    def npc_info(self, msg):
        streamer = Streamer(msg)
        streamer.next_byte()
        npc_id = streamer.next_short()

        posx = streamer.next_float()
        posy = streamer.next_float()
        velx = streamer.next_float()
        vely = streamer.next_float()
        target_player_id = streamer.next_short()

        flags1 = streamer.next_byte()
        flags2 = streamer.next_byte()

        ai = []
        if flags1 & 0x04 != 0: ai.append(streamer.next_byte())
        if flags1 & 0x08 != 0: ai.append(streamer.next_byte())
        if flags1 & 0x10 != 0: ai.append(streamer.next_byte())
        if flags1 & 0x20 != 0: ai.append(streamer.next_byte())

        npc_net_id = streamer.next_short()

        if flags2 & 0x01:
            playerCountForMultiplayerDifficultyOverride = streamer.next_byte()
        if flags2 & 0x04:
            strength_multiplier = streamer.next_float()
        life = None
        if flags1 & 0x80 == 0:
            life_bytes = streamer.next_byte()
            if life_bytes == 1:
                life = streamer.next_byte()
            elif life_bytes == 2:
                life = streamer.next_short()
            elif life_bytes == 4:
                life = streamer.next_int32()
            else:
                logger.error("Bad byte for life bytes flag")

        npc_tilex = int(posx // 16.0)
        npc_tiley = int(posy // 16.0)
        is_npc_in_viewport = npc_tilex > self.viewport_startx and npc_tilex < self.viewport_endx and npc_tiley > self.viewport_starty and npc_tiley < self.viewport_endy
        # logger.debug("npc_info: id={},pos={},tilepos={},vel={},net_id={},life={}".format(npc_id, (posx, posy), (npc_tilex, npc_tiley), (velx, vely), npc_net_id, life))
        # print(self.viewport_startx, self.viewport_starty, self.viewport_endx, self.viewport_endy)
        if is_npc_in_viewport:
            # print("---------------------------in viewport")
            # update npc viewport status map
            pass
    ...
    def read_socket(self):
        ...
        handlers = {
            ...
            MessageType.NPCInfo: self.npc_info,
